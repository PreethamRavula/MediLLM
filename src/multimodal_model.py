...
class MediLLMModel(nn.Module):
    def __init__(
        self, 
        text_model_name="emilyalsentzer/Bio_ClinicalBERT", # Bio_ClinicalBERT is a pretrained model on clinical notes, output to 3 classes i.e triage levels
        num_classes=3, 
        dropout=0.3, 
        hidden_dim=256,
        mode="multimodal"
    ...
    def forward(self, input_ids=None, attention_mask=None, image=None): # input_ids shape: [batch, seq_length], attention_mask: mask to ignore padding same shape, image_tensor: [batch, 3, 224, 224]
      # Text features
      if self.mode == "text":
        ...
        features = text_outputs.last_hidden_state[:, 0, :] # CLS token, return CLS tokens from all batches, position 0, a batch of 3 sentences has 3 CLS tokens 
      ...
      # Image features
        ...
        features = torch.cat((text_feat, image_feat), dim=1) # Concatenates text and image features along feature dimension [CLS vector from BERT] + [ResNet image vector] -> [batch_size, 2816]

      return self.classifier(features) # Return logits for each class, later apply softmax during evaluation
